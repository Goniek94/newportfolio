...
// Syntax-highlighted code block for the challenge question
function CodeBlock() {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex">
        <div className="flex flex-col text-right pr-4 pl-4 py-5 text-[#333] select-none border-r border-[#1a1a1a] text-[13px] leading-7 shrink-0">
          {CHALLENGE.question.split("\n").map((_, i) => (
            <div key={i}>{i + 1}</div>
          ))}
        </div>
        <pre className="flex-1 py-5 px-5 text-[13px] leading-7 overflow-x-auto">
          {CHALLENGE.question.split("\n").map((line, i) => {
...
export default function ChallengeTab() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Answers — compact 2×2 grid below the terminal */}
      <motion.div
      ...
      >
        <p className="text-neutral-500 text-[11px] font-mono uppercase tracking-[0.25em]">
          ...
          {CHALLENGE.options.map((opt) => {
            ...
            return (
              <button
                ...
                disabled={revealed}
                className="flex items-start gap-2.5 px-3.5 py-2.5 rounded-xl border text-left transition-all duration-200 disabled:cursor-default"
                style={{ borderColor, background: bg }}
              >
                <span
                  className="font-mono text-[9px] font-black uppercase tracking-widest mt-0.5 shrink-0 w-3"
                ...
                {revealed && isRight && (
                  <span className="ml-auto text-[#27c93f] shrink-0 text-xs">
                    ✓
                  </span>
                )}
                {revealed && isSelected && !isRight && (
                  <span className="ml-auto text-[#ef4444] shrink-0 text-xs">
                    ✗
                  </span>
                )}
              </button>
        ...
        <div className="flex items-center gap-3">
          {!revealed ? (
            <button
              ...
              disabled={!selected}
              className="px-5 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
              style={{
